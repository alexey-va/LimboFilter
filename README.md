...
Version `1.2.0-ruscrafting.20` adds two independent checks on top of upstream
LimboFilter:
...
one-time-captcha:
  ...
  generator-threads: 1
  memory-grid-preview-millis: 8000
...
`ITEM_SEQUENCE` uses the existing `framed-captcha.coords` as the anchor for its
fixed 3x3 wall even when normal framed CAPTCHA mode is disabled. The top row is
